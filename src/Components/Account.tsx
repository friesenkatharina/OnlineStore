...
function PasswordModal({ onClose }: PasswordModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (next.length < 8) { setError("Neues Passwort muss mind. 8 Zeichen haben."); return; }
    if (!/\d/.test(next)) { setError("Neues Passwort muss mind. eine Zahl enthalten."); return; }
    setLoading(true);
    try {
      ...
      <div className="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-md">
        {success ? (
          <div className="text-center py-4">
            ...
            <h3 className="text-xl font-bold text-gray-900 mb-2">Passwort geändert!</h3>
            <p className="text-sm text-gray-500 mb-6">Dein Passwort wurde erfolgreich aktualisiert.</p>
            <button onClick={onClose} className="px-6 py-2.5 rounded-xl font-semibold text-white hover:opacity-90 transition" style={{ backgroundColor: "#14532d" }}>
              Schließen
            </button>
          </div>
        ) : (
          <>
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-stone-100 text-gray-400 text-xl">×</button>
            </div>
            {error && (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input type="password" required value={current} onChange={(e) => setCurrent(e.target.value)}
                  className="w-full px-4 py-3 rounded-xl border border-stone-200 bg-stone-50 text-gray-800 focus:outline-none focus:ring-2 focus:border-transparent transition" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Neues Passwort</label>
                <input type="password" required placeholder="Mind. 8 Zeichen, 1 Zahl" value={next} onChange={(e) => setNext(e.target.value)}
                  className="w-full px-4 py-3 rounded-xl border border-stone-200 bg-stone-50 text-gray-800 focus:outline-none focus:ring-2 focus:border-transparent transition" />
              </div>
              <div>
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={onClose} className="flex-1 py-3 rounded-xl border border-stone-200 text-gray-600 font-semibold hover:bg-stone-50 transition">
                  Abbrechen
                </button>
                <button type="submit" disabled={loading} className="flex-1 py-3 rounded-xl font-semibold text-white hover:opacity-90 transition disabled:opacity-60" style={{ backgroundColor: "#14532d" }}>
        ...
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          <div className="bg-white rounded-3xl p-6 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"
            ...
            <div>
              ...
              <h1 className="text-2xl font-bold text-gray-900">Hallo, {user?.username}! 👋</h1>
              <p className="text-gray-500 text-sm mt-1">{user?.email}</p>
            </div>
            <Link to="/store" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-white text-sm font-semibold hover:opacity-90 transition" style={{ backgroundColor: "#14532d" }}>
              🛍️ Weiter shoppen
            </Link>
          </div>
              ...
              {storeItems.slice(0, 4).map((item) => (
                <Link to="/store" key={item.id} className="group block">
                  <div className="aspect-square overflow-hidden rounded-2xl bg-stone-100 mb-2">
                    <img src={item.imgUrl.trim()} alt={item.name.trim()} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                  </div>
          ...
          <div className="bg-white rounded-3xl p-6 shadow-sm">
            ...
            <div className="grid sm:grid-cols-3 gap-3">
              {[
                { icon: "📧", label: "Email ändern", desc: user?.email || "", action: undefined },
                { icon: "🔒", label: "Passwort ändern", desc: "Zuletzt geändert: —", action: () => setShowPasswordModal(true) },
